...
import { buildWeekStats } from './insightStats';
import type { ActivityRecord } from '../../../shared/types';
import { record } from '../../../tests/helpers';

/** Runs the engine the way the route does: records → stats → insights. */
const run = (thisWeek: ActivityRecord[], lastWeekKg = 0): ReturnType<typeof computeInsights> =>
  computeInsights({ stats: buildWeekStats(thisWeek), lastWeekKg });

const ids = (thisWeek: ActivityRecord[], lastWeekKg = 0): string[] =>
  run(thisWeek, lastWeekKg).map((insight) => insight.id);
...
describe('computeInsights — rule engine', () => {
  ...
  it('does not suggest transit when car-km is at or below 30', () => {
    expect(ids([record('car_petrol', 30)], 0)).not.toContain('swap-car-for-transit');
  });
  ...
  it('suggests swapping red meat when 4+ red-meat meals are logged', () => {
    ...
    const insight = run(thisWeek, 0).find((i) => i.id === 'swap-red-meat');
  ...
  it('suggests trimming electricity above 35 kWh/week with a 10% saving', () => {
    ...
    const insight = run(thisWeek, 0).find((i) => i.id === 'trim-electricity');
    // 50 × 0.1 × 0.82 = 4.1
  ...
  it('suggests trains over short-haul flights with the per-km delta', () => {
    ...
    const insight = run(thisWeek, 0).find((i) => i.id === 'train-not-plane');
  ...
  it('suggests second-hand shopping above 20 kg CO2e of purchases', () => {
    ...
    const insight = run(thisWeek, 0).find((i) => i.id === 'buy-less-new');
  ...
  it('suggests efficient cooking above 5 kg LPG/week', () => {
    ...
    const insight = run(thisWeek, 0).find((i) => i.id === 'efficient-cooking');
  ...
  it('flags a rising footprint and targets the top category', () => {
    ...
    const insight = run(thisWeek, 10).find((i) => i.id === 'rising-footprint');
  ...
  it('always quantifies: every insight body and saving is specific', () => {
    ...
    for (const insight of run(thisWeek, 100)) {
